...
const Parts = {
  ...
  update: async (req, res) => {
    try {
      const { prod, month, year } = req.body;

      if (!prod || !month || !year) {
        return res.status(400).json({ code: 400, message: "Part, month, and year are required" });
      }

      // Cari data berdasarkan bulan dan tahun
      const updatedPart = await ProductionModel.findOneAndUpdate(
        { month: month, year: year }, // Mencari berdasarkan bulan dan tahun
        { prod: prod }, // Update field prod
        { new: true } // Mengembalikan dokumen yang telah diperbarui
      );

      if (!updatedPart) {
        return res.status(404).json({ code: 404, message: "Data Production not found" });
      }

      res.status(200).json({ code: 200, message: "Success update part", data: updatedPart });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Server error", error });
    }
