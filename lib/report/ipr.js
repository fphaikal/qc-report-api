...
const IPR = {
  get: (req, res) => {
    ...
    if (date === "all") {
      const sql = `SELECT * FROM ipr ORDER BY info_date ASC`;
    ...
    if (date !== "all") {
      const sql = `SELECT * FROM ipr WHERE DATE(info_date) = ? ORDER BY info_date DESC`;
  ...
  getOperator: (req, res) => {
    ...
    const sql = `SELECT * FROM ipr WHERE pic = ? ORDER BY info_date DESC`;
  ...
  create: async (req, res) => {
    try {
      ...
      const sql = `INSERT INTO ipr (info_date, department_section, problem, source, item, customer, description, cause, countermeasure, form_type, pic, start_date, progress, target_due, actual_finish) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      ...
      db.query(
        ...
        (err, result) => {
          ...
          res
            .status(201)
            .json({ code: 201, message: "Success create ipr" });
        }
      );
    } catch (error) {
      console.log(error);
  ...
  delete: (req, res) => {
    try {
      const { id } = req.params;
      const sql = `DELETE FROM ipr WHERE id = ?`;
