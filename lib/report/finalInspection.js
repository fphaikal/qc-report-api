  ...
  chartData: (req, res) => {
    ...
    db.query(sql, (err, result) => {
      ...
      data.forEach(item => {
        ...
        const month = date.toLocaleString('default', { month: 'long' }); // Mendapatkan nama bulan dalam format panjang

        if (!totals[month]) {
          totals[month] = 0;
        }
        totals[month] += item.total;
      });
      ...
      res.status(200).json({ code: 200, message: "Success", data: totals });
    });
  },
