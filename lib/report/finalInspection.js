...
const FinalInspection = {
  ...
  chartData: (req, res) => {
    ...
    if (type === 'namePart') {
      ...
      db.query(sql, (err, result) => {
        ...
        const namePartTotals = result.reduce((acc, item) => {
          const date = new Date(item.inspection_date).toISOString().split("T")[0];
          if (!acc[date]) {
            acc[date] = { name_part: item.name_part, target: 0, actual: 0 };
          }
  
          acc[date].target += item.target;
          acc[date].actual += item.total; // Menggunakan field `ok` sebagai actual
          return acc;
        }, {});
  
        const namePartData = Object.values(namePartTotals);
        res.status(200).json({ code: 200, message: "Success", data: namePartData });
      });
    }
  ...
  create: async (req, res) => {
    try {
      ...
      const persen = Math.round((total / ((target / 480) * menit)));
